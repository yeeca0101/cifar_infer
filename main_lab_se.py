...
parser.add_argument('--ablation', default=False, type=bool, help='Use SENet with LayerNorm')
...
def main(act,act_name,i):
    ...
    net = SENet18(n_classes=args.n_classes,act=act) if not args.ablation else SENet18_LN(n_classes=args.n_classes,act=act)
    ...
    model_type = 'senet' if not args.ablation else 'senet_ln'
    print(model_type,' train') 
...
if __name__ == '__main__':
    ...
    for i in range(1,args.repeat+1):
        for name, activation_fn in acts.items():
            print(f'act : {name}, batch_size : {args.batch_size}')
